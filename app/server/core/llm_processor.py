...
def generate_random_query_with_openai(schema_info: Dict[str, Any]) -> str:
    ...
    try:
        ...
        # Create prompt for generating interesting queries
        prompt = f"""Given the following database schema:
...
Requirements:
- Maximum 2 sentences
- Be creative and vary the query type
- Consider different patterns: aggregations (COUNT, AVG, SUM), filters (WHERE), sorting, limits (TOP N), time-based queries
- If multiple tables exist, occasionally suggest queries that would join tables
- Make queries that showcase the natural language interface capabilities
- Query should be directly answerable from the available data
...
Generate ONE interesting query now (2 sentences maximum):"""

        response = client.chat.completions.create(
            model="gpt-4.1-mini",
            messages=[
                {"role": "system", "content": "You are a helpful assistant that generates interesting database queries. Always respond with just the query text, nothing else."},
                {"role": "user", "content": prompt}
            ],
...
def generate_random_query(schema_info: Dict[str, Any]) -> str:
    ...
    Routes to appropriate LLM provider (OpenAI priority).
    """
